...
public class EmpConnector {
	...
	public void stop() {
		...
		if (httpClient != null) {
			try {
				...
				logger.error("Unable to stop HTTP transport[{}]", parameters.endpoint(), e);
			}
		}
	}
	...
	private Future<Boolean> connect() {
		logger.info("EmpConnector connecting");
		...
		client.handshake(new MessageListener() {
			...
			public void onMessage(Message m) {
				logger.info("client.handshake : {}", m);
				if (!m.isSuccessful()) {
	...
	private void reconnect() {
		if (running.compareAndSet(false, true)) {
			...
			logger.error("The current value of running is not as we expect, this means our reconnection may not happen");
		}
	}
